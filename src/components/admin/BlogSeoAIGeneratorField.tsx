...
const resolveFieldPath = (
  ...
  const localizedPath = keys.find((key) => key.startsWith(`${basePath}.`) && key.split('.').length === 2)
...
export default function BlogSeoAIGeneratorField() {
  ...
  const titlePath = useMemo(() => resolveFieldPath(formFieldState, 'title', localeCode), [formFieldState, localeCode])
  const contentPath = useMemo(() => resolveFieldPath(formFieldState, 'content', localeCode), [formFieldState, localeCode])
  const seoTitlePath = useMemo(() => resolveFieldPath(formFieldState, 'seoTitle', localeCode), [formFieldState, localeCode])
  const seoDescriptionPath = useMemo(
    () => resolveFieldPath(formFieldState, 'seoDescription', localeCode),
    [formFieldState, localeCode],
  )
  const seoKeywordsPath = useMemo(
    () => resolveFieldPath(formFieldState, 'seoKeywords', localeCode),
    [formFieldState, localeCode],
  )

  const titleField = useField<string | Record<string, unknown> | null>({ path: titlePath })
  const contentField = useField<unknown>({ path: contentPath })
  ...
  const seoKeywordsField = useField<string[] | string | null>({ path: seoKeywordsPath })
  ...
  const onGenerate = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/blogs/generate-seo', {
        ...
        body: JSON.stringify({
          locale: localeCode,
          title: extractFirstText(titleField.value),
          content: extractRichTextText(contentField.value),
          seoKeywords: normalizeKeywords(seoKeywordsField.value),
        }),
      })
      ...
      const normalizedKeywords = normalizeKeywords(data.seoKeywords)
      if (normalizedKeywords.length > 0) {
        seoKeywordsField.setValue(normalizedKeywords)
      }
  ...
  return (
    ...
    >
      ...
      {status ? (
        <div style={{ color: 'var(--theme-success-500)', fontSize: '12px', marginTop: '8px' }}>{status}</div>
      ) : null}
      {errorMessage ? (
